...
class LessonSerializer(serializers.ModelSerializer):
    class Meta:
        model = Lesson
        fields = ['id', 'name', 'video_url', 'product']
...
class ProductStatsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Product
        fields = ['id', 'name', 'lessons_count', 'students_count', 'fill_percentage', 'purchase_percentage',
                  'min_users', 'max_users']
    ...
    def get_fill_percentage(self, obj):
        # Расчет среднего процента заполненности групп продукта
        groups = obj.groups.annotate(fill_level=Count('users') * 100.0 / models.F('max_users'))
        return groups.aggregate(Avg('fill_level'))['fill_level__avg'] or 0

    def get_purchase_percentage(self, obj):
        # Расчет процента пользователей, приобретших продукт, от общего числа пользователей
        total_users = User.objects.count()
        users_with_access = Access.objects.filter(product=obj).distinct().count()
